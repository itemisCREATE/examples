...
static sc_boolean check_main_On_r_Process_Recipe_lr2_lr2(const CoffeeMachine* handle);
static sc_boolean check_main_On_r_Process_Recipe_tr0_tr0(const CoffeeMachine* handle);
...
static void effect_main_On_r_Process_Recipe_lr2_lr2(CoffeeMachine* handle);
...
static void enseq_main_On_r_Process_Recipe_default(CoffeeMachine* handle);
...
static void react_main_On_r_Process_Recipe(CoffeeMachine* handle);
...
void coffeeMachine_runCycle(CoffeeMachine* handle)
{
	...
	for (handle->stateConfVectorPosition = 0;
		...
		{
			react_main_On_r_Process_Recipe(handle);
			break;
		}
		default:
			break;
...
void coffeeMachine_raiseTimeEvent(CoffeeMachine* handle, sc_eventid evid)
{
	...
	switch (state)
	{
		...
		case CoffeeMachine_main_On :
			result = (sc_boolean) (handle->stateConfVector[SCVI_COFFEEMACHINE_MAIN_ON] >= CoffeeMachine_main_On
				&& handle->stateConfVector[SCVI_COFFEEMACHINE_MAIN_ON] <= CoffeeMachine_main_On_r_Process_Recipe);
			break;
		case CoffeeMachine_main_On_r_Welcome :
...
static sc_boolean check_main_On_r_Process_Recipe_tr0_tr0(const CoffeeMachine* handle)
{
	return sc_isFinal(handle->internal.recipeProc);
}
...
{
	exseq_main_On_r_Wait_For_Choice(handle);
	enseq_main_On_r_Process_Recipe_default(handle);
}
...
static void effect_main_On_r_Process_Recipe_lr2_lr2(CoffeeMachine* handle)
{
	sc_runCycle(handle->internal.recipeProc);
}
...
static void enact_main_On_r_Process_Recipe(CoffeeMachine* handle)
{
	...
	handle->internal.processing = bool_true;
	handle->internal.recipeProc = &(recipeProcessors[handle->internal.userInput - 1]);
	sc_enter(handle->internal.recipeProc);
}
...
static void exact_main_On_r_Process_Recipe(CoffeeMachine* handle)
{
	/* Exit action for state 'Process Recipe'. */
	handle->internal.processing = bool_false;
	sc_exit(handle->internal.recipeProc);
}
...
/* 'default' enter sequence for state Process Recipe */
static void enseq_main_On_r_Process_Recipe_default(CoffeeMachine* handle)
{
	/* 'default' enter sequence for state Process Recipe */
	enact_main_On_r_Process_Recipe(handle);
	coffeeMachine_stateEntered(handle, CoffeeMachine_main_On_r_Process_Recipe);
	handle->stateConfVector[0] = CoffeeMachine_main_On_r_Process_Recipe;
	handle->stateConfVectorPosition = 0;
}
...
/* The reactions of state Process Recipe. */
static void react_main_On_r_Process_Recipe(CoffeeMachine* handle)
{
	/* The reactions of state Process Recipe. */
	effect__lr0(handle);
	...
	{
		if (check_main_On_r_Process_Recipe_tr0_tr0(handle) == bool_true)
		{ 
			effect_main_On_r_Process_Recipe_tr0(handle);
		}  else
		{
			effect_main_On_r_Process_Recipe_lr2_lr2(handle);
		}
	}
}
