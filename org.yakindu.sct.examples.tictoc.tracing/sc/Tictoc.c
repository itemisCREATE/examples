...
static void runCycle(Tictoc* handle);



static void tictoc_eventqueue_init(tictoc_eventqueue * eq, tictoc_event *buffer, sc_integer capacity);
static sc_integer tictoc_eventqueue_size(tictoc_eventqueue * eq);
static void tictoc_event_init(tictoc_event * ev, TictocEventID name);
static tictoc_event tictoc_eventqueue_pop(tictoc_eventqueue * eq);
static sc_boolean tictoc_eventqueue_push(tictoc_eventqueue * eq, tictoc_event ev);
static void tictoc_add_event_to_queue(tictoc_eventqueue * eq, TictocEventID name);
static void tictoc_dispatch_event(Tictoc* handle, const tictoc_event * event);
static tictoc_event tictoc_get_next_event(Tictoc* handle);
static void tictoc_dispatch_next_event(Tictoc* handle);
static TictocEventID tictoc_get_timed_event_name(Tictoc* handle, sc_eventid evid);
...
void tictoc_initWithTracing(Tictoc* handle, sc_trace_handler *trace_handler)
{
	...
	sc_observable_init(&handle->iface.tic);
	sc_observable_init(&handle->iface.toc);
	...
	tictoc_eventqueue_init(&handle->in_event_queue, handle->in_buffer, TICTOC_IN_EVENTQUEUE_BUFFERSIZE);
...
void tictoc_raiseTimeEvent(Tictoc* handle, sc_eventid evid)
{
	...
	{
		tictoc_add_event_to_queue(&(handle->in_event_queue), tictoc_get_timed_event_name(handle, evid));
...
static void runCycle(Tictoc* handle)
{
	...
	tictoc_dispatch_next_event(handle);
	do
	{ 
		...
		clearInEvents(handle);
		tictoc_dispatch_next_event(handle);
	} while (((((handle->iface.toggle_raised == bool_true) || (handle->timeEvents.tictoc_main_Idle_tev0_raised == bool_true)) || (handle->timeEvents.tictoc_main_Playing_tev0_raised == bool_true)) || (handle->timeEvents.tictoc_main_Playing_active_Tic_tev0_raised == bool_true)) || (handle->timeEvents.tictoc_main_Playing_active_Toc_tev0_raised == bool_true));
...
static void enact_main_Playing_active_Tic(Tictoc* handle)
{
	...
	sc_observable_next(&handle->iface.tic)
	;
}
...
static void enact_main_Playing_active_Toc(Tictoc* handle)
{
	...
	sc_observable_next(&handle->iface.toc)
	;
	handle->iface.x += 1;
	SC_TRACE_FEATURE(handle, sc_trace_variable_set, tictocIface_x, &handle->iface.x);
}
...
{
	tictoc_event event;
	if(tictoc_eventqueue_size(eq) <= 0) {
		tictoc_event_init(&event, Tictoc_InvalidEvent);
	}
	else {
...
static sc_boolean tictoc_eventqueue_push(tictoc_eventqueue * eq, tictoc_event ev)
{
	if(tictoc_eventqueue_size(eq) == eq->capacity) {
		return bool_false;
	}
	else {
...
{
	tictoc_event event;
	tictoc_event_init(&event, name);
	tictoc_eventqueue_push(eq, event);
}

static void tictoc_dispatch_event(Tictoc* handle, const tictoc_event * event) {
	switch(event->name) {
		case TictocIface_toggle:
		{
		...
		{
			handle->timeEvents.tictoc_main_Idle_tev0_raised = bool_true;
			break;
		}
		case tictoc_main_Playing_time_event_0:
		{
			handle->timeEvents.tictoc_main_Playing_tev0_raised = bool_true;
			break;
		}
		case tictoc_main_Playing_active_Tic_time_event_0:
		{
			handle->timeEvents.tictoc_main_Playing_active_Tic_tev0_raised = bool_true;
			break;
		}
		case tictoc_main_Playing_active_Toc_time_event_0:
		{
			handle->timeEvents.tictoc_main_Playing_active_Toc_tev0_raised = bool_true;
			break;
		}
		default:
			break;
...
static tictoc_event tictoc_get_next_event(Tictoc* handle)
{
	tictoc_event next_event;
	tictoc_event_init(&next_event, Tictoc_InvalidEvent);
	if(tictoc_eventqueue_size(&(handle->in_event_queue)) > 0) {
		next_event = tictoc_eventqueue_pop(&(handle->in_event_queue));
	}
	return next_event;
}
...
static void tictoc_dispatch_next_event(Tictoc* handle)
{
	tictoc_event nextEvent;
	nextEvent = tictoc_get_next_event(handle);
	tictoc_dispatch_event(handle, &nextEvent);
}

static TictocEventID tictoc_get_timed_event_name(Tictoc* handle, sc_eventid evid)
{
	if(evid == &handle->timeEvents.tictoc_main_Idle_tev0_raised) {
		return tictoc_main_Idle_time_event_0;
	}
	if(evid == &handle->timeEvents.tictoc_main_Playing_tev0_raised) {
		return tictoc_main_Playing_time_event_0;
	}
	if(evid == &handle->timeEvents.tictoc_main_Playing_active_Tic_tev0_raised) {
		return tictoc_main_Playing_active_Tic_time_event_0;
	}
	if(evid == &handle->timeEvents.tictoc_main_Playing_active_Toc_tev0_raised) {
		return tictoc_main_Playing_active_Toc_time_event_0;
	}
	return Tictoc_InvalidEvent;
}
